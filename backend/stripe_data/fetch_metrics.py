...
async def get_stripe_metrics(project_id: str, request: FastAPIRequest, date: str = None):
    ...
    try:
        ...
        # If account_name is still "Unknown Account", get it from Stripe
        if account_name == "Unknown Account":
            try:
                account = stripe.Account.retrieve()
                account_name = account.get("business_profile", {}).get("name") or account.get("display_name") or f"Stripe Account {account.get('id', '')[:10]}"
                logging.info(f"Retrieved account name: {account_name}")
            except Exception as e:
                logging.warning(f"Could not get account name: {str(e)}")
...
# Background task to fetch Stripe data after OAuth connection
async def background_stripe_fetch(user_id: str, project_id: str):
    """Use EXACT same logic as manual fetch - all 41 metrics"""
    try:
        logging.info(f"Starting background Stripe data fetch for user {user_id}, project {project_id}")
        ...
        if not result.data:
            logging.error("No Stripe credentials found for background fetch")
            return
        ...
        # Collection of metrics (SAME as manual fetch)
        metrics = []
        ...
        try:
            ...
            metrics.append({
                ...
                "date": target_date_str,
                "metric_name": "available_balance",
                "metric_value": available_balance / 100,
                "account_name": account_name,
                "metric_description": "Available balance in Stripe account"
            })
        ...
        # Store metrics in database using UPSERT logic
        stored_count = 0
        for metric in metrics:
            try:
                # Check if metric already exists for this date
        ...
        logging.info(f"✅ Background Stripe data fetch completed: {stored_count} metrics stored for {account_name}")
        
    except Exception as e:
        logging.error(f"Error in background Stripe data fetch: {str(e)}")